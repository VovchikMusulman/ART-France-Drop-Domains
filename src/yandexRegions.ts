...
export const YANDEX_COUNTRIES: YandexRegion[] = [
  { id: 225, name: 'Россия' },
  { id: 187, name: 'Украина' },
...
export const YANDEX_CITIES: YandexRegion[] = [
...
export const DEFAULT_YANDEX_REGION_ID = 213;

export function findYandexRegionName(id: number | string | undefined | null): string {
  const n = Number(id);
  if (!Number.isFinite(n)) return '';
  const hit = YANDEX_COUNTRIES.find((r) => r.id === n) || YANDEX_CITIES.find((r) => r.id === n);
  return hit?.name || String(n);
}
